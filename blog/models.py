...
class post(models.Model):
    ...
    author = models.ForeignKey(User, verbose_name=_(
        "نویسنده"), on_delete=models.SET_NULL, null=True)
    image = models.ImageField(
        _("عکس"), upload_to='blog/', default='blog/default.jpg')
    title = models.CharField(_("عنوان"), max_length=255)
    content = models.TextField(_("مطالب"))
    tags = TaggableManager()
    Type_Food = models.CharField(
        _("نوع غذا"), max_length=50, choices=CHOICES, default='drinks')
...
class comment(models.Model):
    ...
    update_date = models.DateTimeField(
        _("تاریخ بروز رسانی"), auto_now_add=True)
